...
Что работает по коду и по записям в `docs/PROJECT-STATUS.md` (проверялось вживую на 2026-06-23, **с тех пор не
перепроверялось**):
...
По записи в `docs/PROJECT-STATUS.md` (2026-06-14) официальное **ТЗ v2 так и не пришло** — зав. кафедрой обещал «в
...
ASP.NET Core, а не 5080 — все записи в `docs/PROJECT-STATUS.md`/`ONBOARDING.md` подразумевают именно 5080.
...
Вывод, добытый экспериментом (записан в `docs/PROJECT-STATUS.md`): **«зум вниманием» надёжнее пиксельного кропа** —
...
- **Рабочий код — только в `app/`.** Ранний несобиравшийся скаффолд удалён из репозитория:
  все 22 его файла дословно повторяли пути в `app/`, уникального в нём не было ничего.
...
| Файл | Что внутри |
|---|---|
| `docs/PROJECT-STATUS.md` | текущий статус в деталях, что сделано в последних сессиях, ключевые факты, не выводимые из кода |
...
(строки 68–69) и в `docs/PROJECT-STATUS.md` — но в официальном ТЗ v1 их нет, и в коде их нет: это план. Концепт «Кафедральный
