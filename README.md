...
- карточка студента `/students/{id}` — текущая справка, история, правка `/certificates/{certId}/edit`, отзыв допуска;
- экран «Перед парой» `/beforeclass`;
- загрузка скана студентом (`/me`) и преподавателем, склейка нескольких фото в один PDF;
- фоновое ИИ-распознавание (двухэтапное + голосование по дате), ручной зум по полю `/scans/{scanId}/zoom`;
- журнал аудита `/journal`, импорт реестра `/import`.
...
**Вход.** Bootstrap-пользователь создаётся, если `BootstrapUser:Enabled = true`. В
`appsettings.json` он **выключен**, в `appsettings.Development.json` — **включён**, логин `teacher`,
пароль — дефолтный `<демо-пароль>` из того же файла. `SeedDemoData = false` → **реестр стартует пустым,
это норма**, демо-студентов сознательно убрали по просьбе заказчика. Справочники (9 высших школ,
76 преподавателей, 2 учебные группы) сидятся всегда.
...
| Аудит действий | таблица `audit_logs`, append-only по замыслу (`AuditEntryFactory`) |
