...
3. Твой SSH-ключ уже добавлен в аккаунт `Castiel`. Подключение:
   ```bash
   ssh Castiel@<tailscale-ip>
   ```
   **Важно:** заходи ОБЯЗАТЕЛЬНО под пользователем `Castiel` (с большой C), не под своим именем.
   **Про IP:** у владельца нода видна как `<tailscale-ip>`, но у тебя (расшаренная нода) Tailscale может
   показать её под другим адресом (например `<tailscale-ip>`) — используй тот IP, что показывает твой
...
- Логин преподавателя: **`teacher`** / **`<демо-пароль>`** (создаётся всегда).
...
- **Дефолтный пароль `<демо-пароль>` на публичном адресе** — по-хорошему сменить (медданные за слабым логином).
...
- `CLAUDE.md` — текущий статус проекта (детально, что сделано в последних сессиях).
