...
public static class DataSeeder
{
    public static async Task SeedIdentityAsync(this IServiceProvider services, IConfiguration configuration)
    {
        ...
        if (configuration.GetValue<bool>("SeedDemoData"))
        {
            ...
            await EnsureDemoUserAsync(userMgr, "head", "Егоров Максим Олегович", AppRoles.HeadOfDepartment);
    ...
    private static async Task SeedOnecDemoTableAsync(ApplicationDbContext db)
    {
        ...
        await db.Database.ExecuteSqlRawAsync(@"
            ...
            ('OD-1004','Преподаватель 47','Высшая школа менеджмента',2,'15.26Д-ММО01/24б','Сергеева Ирина Павловна'),
            ('OD-1005','Преподаватель 18','Высшая школа менеджмента',2,'15.26Д-ММО01/24б','Сергеева Ирина Павловна');");
    }
    ...
    // Демонстрационный преподавательский состав.
    //
    // Здесь намеренно НЕТ настоящей росписи кафедры: справочник реальных людей —
    // это персональные данные, и в коде приложения им не место. Боевая установка
    // загружает свой список через импорт Excel (`/import`) либо заводит
    // преподавателей через интерфейс; сид нужен только чтобы пустая база сразу
    // выглядела рабочей.
    private static readonly (string Name, string Position)[] TeacherSeed =
    {
        ("Преподаватель 40", "Доцент"),
        ("Преподаватель 57", "Доцент"),
        ("Сергеева Ирина Павловна", "Преподаватель"),
        ("Никитин Артём Владимирович", "Преподаватель"),
        ("Морозова Елена Сергеевна", "Преподаватель"),
        ("Козлов Павел Андреевич", "Преподаватель"),
        ("Волкова Анна Дмитриевна", "Преподаватель"),
        ("Соколов Денис Игоревич", "Преподаватель"),
        ("Лебедева Ольга Викторовна", "Преподаватель"),
        ("Егоров Максим Олегович", "Профессор, зав. кафедрой"),
        ("Тимофеева Мария Алексеевна", "Преподаватель"),
        ("Данилов Роман Сергеевич", "Преподаватель"),
    };
}
